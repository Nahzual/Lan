...
@section('title')
All teams for the tournament {{ $ntourn }}
@endsection
...
@section('title-buttons')
	<div class="col mt-1">
		<form method="GET" action="{{ route('team.create_team', $tournamentId) }}">
			@csrf
			<button type="submit" class="btn btn-outline-success float-right"><i class='fa fa-plus-square'></i> Add a team</button>
		</form>
	</div>
@endsection
...
<nav aria-label="page navigation">
	<ul class="pagination justify-content-end">
		<li class="page-item">
			<a class="btn btn-info" href="{{ url('/tournament/'.$tournamentId) }}" tabindex="-2">Return to the Tournament</a>

			<li class="page-item">
				<a class="btn btn-secondary" href="{{ url('/tournament/'.$tournamentId.'/teams/') }}" tabindex="-2">First</a>
			</li>
			<li class="page-item @if($previous == 0) disabled @endif">
				<a class="btn btn-outline-info" href="{{ $previous!=0 ?  url('/tournament/'.$tournamentId.'/teams/'.($previous)) : '#' }}" tabindex="-1">Back</a>
			</li>
			...
			<li class="page-item @if($next) @else disabled @endif">
				<a class="btn btn-outline-info" href="{{ $next ? url('/tournament/'.$tournamentId.'/teams/'.($next)) : '#' }}">Next</a>
			</li>
			<li class="page-item">
				<a class="btn btn-secondary" href="{{ url('/tournament/'.$tournamentId.'/teams/'.$max) }}">Last</a>
			</li>
		</ul>
	</nav>
